...
module.exports.run = async (client, msg, args, config) => {
    if(cooldown.has(msg.author.id)) {
        ...
        fs.readFile('./accounts/amazon.txt', 'utf8', function(err, data) {
            if (err) throw err;
            ...
            fs.writeFile('./accounts/amazon.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });

            let embed = new Discord.RichEmbed()
            .addField('Support Server', `https://discord.gg/CDVDzWF`)
            .addField('amazon sc card ',`\n**${account}**`)
            ...
            msg.reply('1x **amazon** sc card sent in DM!')
                .then(m => {
                    setTimeout(() => {
                        m.delete();
                    }, 900000);
...
module.exports.help = {
    name: `amazon`,
    description: `Sends you a amazon sc card`
};