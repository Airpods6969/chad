...
module.exports.run = async (client, msg, args, config) => {
    if(cooldown.has(msg.author.id)) {
        ...
        fs.readFile('./accounts/wish.txt', 'utf8', function(err, data) {
            if (err) throw err;
            ...
            fs.writeFile('./accounts/wish.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });
            ...
            msg.reply('1x **wish** account sent in DM!')
                .then(m => {
                    setTimeout(() => {
                        m.delete();
                    }, 900000);
...
module.exports.help = {
    name: `wish`,
    description: `Sends you a wish account`
};