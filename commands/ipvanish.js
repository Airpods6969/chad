...
module.exports.run = async (client, msg, args, config) => {
    if(cooldown.has(msg.author.id)) {
        ...
        fs.readFile('./accounts/ipvanish.txt', 'utf8', function(err, data) {
            if (err) throw err;
            ...
            fs.writeFile('./accounts/ipvanish.txt', lines.slice(1).join('\n'), function(err) {
                if(err) throw err;
            });

            let embed = new Discord.RichEmbed()
            .addField('Support Server', `https://discord.gg/CDVDzWF`)
            .addField('ipvanish account',`\n**${account}**`)
            ...
            msg.reply('1x **ipvanish** Account sent in DM!')
                .then(m => {
                    setTimeout(() => {
                        m.delete();
                    }, 900000);
...
module.exports.help = {
    name: `ipvanish`,
    description: `Sends you a ipvanish account!`
};